...
# Map selected cuisines to model input
cuisine_data = {col: 0 for col in all_cuisine_columns}
for cuisine in selected_cuisines:
    col_name = f"primary_cuisine_{cuisine}"
    if col_name in cuisine_data:
...
if st.sidebar.button("🚀 Predict Rating"):
    input_data = pd.DataFrame([{
        ...
        "rest_type": rest_type_map[rest_type_name],
        "listed_in_type": listed_type_map[listed_in_type_name],
        "listed_in_city": listed_city_map[listed_in_city_name],
        "online_order": online_order,
        "book_table": book_table,
        "ave_cost_for_two": ave_cost_for_two,
        "votes": votes,
